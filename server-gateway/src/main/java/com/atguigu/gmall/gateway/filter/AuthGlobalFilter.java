...
public class AuthGlobalFilter implements GlobalFilter {
    ...
    /**
     *
     * @param exchange spring 框架封装的 web 服务请求 request与响应 response对象
     * @param chain 过滤器链对象
     * @return
     */
    @Override
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        String path = request.getURI().getPath(); // /api/product/inner/getSkuInfo/28
        //  判断是否属于内部数据接口
        ...
        //  获取到登录的userId,在缓存中获取数据， 必须要有token ,token 存储在header 或 cookie
        ...
        if (split!=null && split.length>0){
            for (String url : split) {
                //  path 包含上述控制器 并且 用户Id 为空 ,此时需要拦截并跳转
        ...
        //  将获取到的用户Id 添加到请求头： 请求头可能会存有userId!  OOP 思想。
        //  以后使用这个类AuthContextHolder 来获取用户Id
        if (!StringUtils.isEmpty(userId) || !StringUtils.isEmpty(userTempId)){
            if (!StringUtils.isEmpty(userId)) {
                //  放入请求头：   ServerHttpRequest request = exchange.getRequest();
    ...
    private String getUserId(ServerHttpRequest request) {
        ...
        if (!StringUtils.isEmpty(token)){
            ...
            if (!StringUtils.isEmpty(strJson)){
                //  "{\"ip\":\"127.0.0.1\",\"userId\":\"2\"}"  JSONObject
